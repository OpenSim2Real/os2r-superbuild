...
public:
   ...
   */
  mutex() { rt_mutex_create(&rt_mutex_, NULL); }

  /**
   * @brief lock the mutex.
   */
  void lock() { rt_mutex_acquire(&rt_mutex_, TM_INFINITE); }

  /**
   * @brief Unlock the mutex.
   */
  void unlock() { rt_mutex_release(&rt_mutex_); }
};
...
public:
   ...
   */
  condition_variable() { rt_cond_create(&rt_condition_variable_, NULL); }
  ...
  void wait(std::unique_lock<mutex> &lock) {
    rt_cond_wait(&rt_condition_variable_, &lock.mutex()->rt_mutex_,
                 TM_INFINITE);
  }
   ...
   */
  void notify_all() { rt_cond_broadcast(&rt_condition_variable_); }
};
} // namespace xenomai
/**
 ...
 * @param tolen
 */
inline void send_to_can_device(int fd, const void *buf, size_t len, int flags,
 ...
 * @param flags
 */
inline void receive_message_from_can_device(int fd, struct msghdr *msg,
                                            int flags) {
  int ret = rt_dev_recvmsg(fd, msg, flags);
  if (ret < 0) {
    std::ostringstream oss;
    oss << "something went wrong with receiving "
        << "CAN frame, error code: " << ret << ", errno=" << errno << std::endl;
    throw std::runtime_error(oss.str());
  }
}
