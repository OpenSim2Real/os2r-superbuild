...
template <int ORDER> Polynome<ORDER>::Polynome() { coefficients_.fill(0.0); }

template <int ORDER> Polynome<ORDER>::~Polynome() {}

template <int ORDER> double Polynome<ORDER>::compute(double x) {
  double res = 0.0;
  double pt = 1.0;
...
template <int ORDER> double Polynome<ORDER>::compute_derivative(double x) {
  double res = 0.0;
  double pt = 1.0;
...
template <int ORDER> double Polynome<ORDER>::compute_sec_derivative(double x) {
  double res = 0.0;
  double pt = 1.0;
...
template <int ORDER> void Polynome<ORDER>::print() const {
...
template <int ORDER> double TimePolynome<ORDER>::compute(double t) {
  if (t <= 0.0) {
    return init_pose_;
...
template <int ORDER> double TimePolynome<ORDER>::compute_derivative(double t) {
  if (t <= 0.0) {
    return init_speed_;
  } else if (t >= final_time_) {
    return final_speed_;
